...
### Different ways to approximate cancer-death relationship

```{r}
fixed.dat$cancerToDeathRate = fixed.dat$cancerRate / fixed.dat$realDeathRate
fixed.dat$DeathToCancerRate = 1/fixed.dat$cancerToDeathRate
summary(fixed.dat$cancerToDeathRate)
summary(fixed.dat$DeathToCancerRate)
```

Looking at probabilities, although here we can't say that cause of death was cancer. We define cancer mortality rate as number of deaths due to cancer (or closest possible approximation) per 100,000 population

```{r}
fixed.dat$prob_of_cancer_and_death = (fixed.dat$avgAnnCount/fixed.dat$popEst2015) *
  ((fixed.dat$popEst2015 * fixed.dat$deathRate/100000)/fixed.dat$popEst2015)
fixed.dat$cancer_mortality_rate=100000*(fixed.dat$prob_of_cancer_and_death * fixed.dat$popEst2015)/fixed.dat$popEst2015
summary(fixed.dat$cancer_mortality_rate)
```
...
```{r}
scatterplotMatrix(~ cancerRate + realDeathRate, data = fixed.dat)
```
